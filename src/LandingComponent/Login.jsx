...
import HrLayout from "../Component/Layout/HrLayout";
import HeadHrLayout from "../Component/Layout/HeadHrLayout";
import HeadAgentLayout from "../Component/Layout/HeadAgentLayout";
import AgentLayout from "../Component/Layout/AgentLayout";
import VerifierLayout from "../Component/Layout/VerifierLayout";
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:1230/register/checkCredentials", {
        userName: '',
        email: userEmail,
      ...
      if (responseData[0] === "validCredentials" && responseData[1]) {
        ...
        const userId = responseData[3];
        const company = responseData[4];
        ...
        await axios.post("http://localhost:1230/register/addToken", {
          email: userEmail,
          password: userPassword,
  ...
  if (isLoggedIn) {
    // Conditionally render PatientLayout based on user role
    if (userRole === 'hr') {
      console.log("hrlayoutinvoked");
      return <HrLayout />
    } else if(userRole === 'headhr'){
      return <HeadHrLayout/>
    } else if(userRole ==='headagent'){
      return <HeadAgentLayout/>
  ...
  return (
    <section className="h-screen flex flex-col md:flex-row justify-center space-y-10 md:space-y-0 md:space-x-16 items-center my-2 mx-5 md:mx-0 md:my-0">
      <div className="md:w-1/3 max-w-sm mr-20">
        <img src="employeeLogin.jfif" alt="Sample image" />
      </div>
